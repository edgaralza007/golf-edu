  ...
  if (!drill) {
    return (
      <div className="max-w-3xl mx-auto text-center py-16">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Drill Not Found</h1>
        <p className="text-gray-500 mb-6">The drill you are looking for does not exist.</p>
        <Link
          to="/drills"
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link to="/drills" className="hover:text-green-700 transition-colors">
          Practice Drills
        </Link>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        <span className="text-gray-900 font-medium">{drill.name}</span>
      </nav>

      {/* Header */}
      <div className="mb-6">
        <div className="flex items-start justify-between gap-3 mb-3">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{drill.name}</h1>
          <button
        ...
        <ol className="space-y-4">
          {drill.steps.map((step, i) => (
            <li key={i} className="flex gap-3">
              <span className="flex items-center justify-center w-7 h-7 rounded-full bg-green-100 text-green-800 text-sm font-bold shrink-0">
                {i + 1}
              </span>
      ...
      <Card className="mb-4 bg-green-50 border-green-200">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-green-900 mb-1">Success Criteria</h2>
            <p className="text-sm text-green-800 leading-relaxed">{drill.successCriteria}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="mb-6">
        {completed ? (
          <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-green-100 text-green-800 text-sm font-medium">
      ...
      {drill.relatedSwingType && (
        <Card className="mb-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-gray-900">Related Swing Lesson</h2>
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg border border-green-200 text-green-700 text-sm font-medium hover:bg-green-50 transition-colors"
            >
              View Lesson
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {related.map((d) => (
              <Link
                key={d.id}
                to={`/drills/${d.id}`}
                className="flex items-start gap-3 p-4 rounded-xl bg-white border border-gray-100 hover:border-green-200 hover:shadow-sm transition-all"
              >
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium text-gray-900 truncate">{d.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <DifficultyBadge level={d.difficulty} />
                    <span className="text-xs text-gray-500">{d.estimatedMinutes} min</span>
                  </div>
                </div>
                <svg
